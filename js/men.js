...
function displayValues(val1,val2,val3){
    ...
    reqjs(val1,val2,val3,value4,function (data) {
        if(!data){
            return 0;
        }else{
...
function deleteLoadedShoes(){
    ...
    window.onload = function () {
        ...
        reqjs(0, 0, 0,0,function (callbackValue){
 ...
 * *******************
 * Description:
 * This function is used to handle click event on pagination
 * listen for click event on pagination
 */
function reqForPaginating(page,callback){
    page = encodeURIComponent(JSON.stringify(page));
    const xhr = new XMLHttpRequest();
    const url = `http://localhost/pp2/p5/control/reqdata.php?page=${page}`;
...
function handleClick(event){
    ...
    innerHtml = sanitizeInput(innerHtml);
    if(innerHtml === "Previous" || innerHtml === "Next" || (innerHtml >=1 && innerHtml <=100)) {
        console.log(innerHtml);
        console.log("Proceed");
        reqForPaginating(innerHtml,function (callBackValue){
            deleteLoadedShoes();
            // console.log(callBackValue);
            console.log("**********callbackValue************");
            console.log(callBackValue);
            console.log("********** type of ************");
            // let hello = JSON.parse(callBackValue);
            console.log(typeof callBackValue)
            /*callBackValue = JSON.parse(callBackValue);
            callBackValue.forEach(item=>{
                load(item.name,item.price,item.imgurl);
            })*/
            console.log("length of callback Value for paginating : " + callBackValue.length);
            console.log(callBackValue);

        });
    }else{
        console.log("something went is wrong");
    }
}

// queyr selecterall က a.page-link ရဲ့ parent element ကိုယူလိုက်တယ် that's why
//listItems function (li) ကိုသုံးတယ်။



var listItems = document.querySelectorAll('.page-link');
...
function checkValue(value){
    const xhr = new XMLHttpRequest();
    const url = `http://localhost/pp2/p5/control/reqdata.php?value=${value}`;
    xhr.open('GET', url, true);
